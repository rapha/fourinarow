...
let _ =
  ...
  ("AI" >::: [
      ...
      "minimax returns losing score if opponent has won" >:: (fun() ->
      ...
      "minimax with depth 1 returns winning score if player can win this turn" >:: (fun() ->
        let module TestAI = Ai.Make (struct include Board
          let wins player board =
            player = Player.A
        end) in
      ...
      "minimax with depth 1 returns highest values from eval function after this turn" >:: (fun() ->
        let module TestAI = Ai.Make (struct include Board
