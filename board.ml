module Make (Player : sig
  type t = Player.t
  val name : Player.t -> string
  val eq : Player.t -> Player.t -> bool
  val to_string : Player.t -> string
  val of_string : string -> Player.t option
end) = struct
  ...
  let build rows =
    let cols = rows |> List.map (Str.split (Str.regexp "") |- List.map Player.of_string) |> List.transpose row_length in
    Board cols
